...
export default function EstimationEtape2Page() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Nombre de photos à uploader:', photos.length)
      if (photos.length > 0) {
        // On utilise un ID temporaire pour le dossier Storage
        ...
        photoUrls = await uploadEstimationPhotos(photos, tempId, (uploaded, total) => {
          console.log(`Photo ${uploaded}/${total} uploadée`)
          setUploadProgress(`Upload des photos (${uploaded}/${total})...`)
        })
        console.log('URLs des photos obtenues:', photoUrls.length)
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="space-y-6">
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <div className="grid md:grid-cols-3 gap-4 mt-4">
                {[
                  'Double vitrage', 'Climatisation', 'Cuisine équipée', 'Cave', 'Local vélo', 'Cheminée',
                  'Eau chaude', 'Eau froide', 'Chauffage collectif', 'Chauffage individuel', 'Chauffage au sol',
                  'Volets électriques', 'Volets manuels', 'Piscine', 'Immeuble sécurisé',
                  'Ascenseur', 'Fibre optique', 'Alarme', 'Porte blindée'
                ].map((option) => (
              ...
              <div className="grid md:grid-cols-5 gap-4 mt-4">
                {['Nord', 'Sud', 'Est', 'Ouest', 'Traversant'].map((option) => (
                  <label key={option} className={getOptionClass(formData.exposition === option)}>
                    <input type="radio" name="exposition" value={option} checked={formData.exposition === option} onChange={handleChange} className="mr-2 accent-white" />
                    <span className="text-white text-sm" style={fontStyle}>{option}</span>
                  </label>
                ))}
              </div>
              {formData.exposition === 'Traversant' && (
                <div className="grid md:grid-cols-2 gap-4 mt-4">
                  {['Nord / Sud', 'Est / Ouest'].map((option) => (
                    <label key={option} className={getOptionClass(formData.expositionTraversant === option)}>
                      <input type="radio" name="expositionTraversant" value={option} checked={formData.expositionTraversant === option} onChange={handleChange} className="mr-2 accent-white" />
                      <span className="text-white text-sm" style={fontStyle}>{option}</span>
                    </label>
                  ))}
                </div>
                ...
                <div className="mt-4 space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    {['Nue', 'Meublée'].map((option) => (
                      <label key={option} className={getOptionClass(formData.typeLocation === option)}>
                        <input type="radio" name="typeLocation" value={option} checked={formData.typeLocation === option} onChange={handleChange} className="mr-2 accent-white" />
                        <span className="text-white text-sm" style={fontStyle}>{option}</span>
                      </label>
                    ))}
                  </div>
                  <div>
                    <label className={labelClass} style={fontStyle}>Loyer mensuel</label>
                    <input type="text" name="loyerMensuel" value={formData.loyerMensuel} onChange={handleChange} placeholder="Ex: 800 €" className={inputClass} style={fontStyle} />
                  </div>
                </div>
              )}
            </div>
